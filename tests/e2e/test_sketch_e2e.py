...
def test_extrude_sketch_end_to_end_produces_extrude_spec() -> None:
...
def test_revolve_sketch_end_to_end_produces_revolve_spec() -> None:
...
def test_untagged_sketch_still_produces_sketch_spec() -> None:
    ...
    # An untagged sketch is skipped by the extractor since we don't know how to build it.
    assert spec is None, "Extractor emitted a spec for untagged sketch, expected None"


def test_extrude_sketch_with_wrong_point_count_emits_unsupported() -> None:
...
def test_revolve_sketch_with_wrong_point_count_emits_unsupported() -> None:
