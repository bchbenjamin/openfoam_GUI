...
1. In the sidebar, find the **Case Settings** section.
2. Click the folder icon next to the **Case Path** field and navigate to an existing directory (or create one, e.g., `~/openfoam_cases/test_case`).
**✓ Checkpoint:** The field shows the path you chose. If the **▶ Run All** button at the bottom gives a "No Case Directory" popup when clicked, the path is empty or not set.
---
## Part 2: Adding Primitives
The simplest way to add a block is to use the **Add Classy Primitives** section at the top of the sidebar (it only appears in Object Mode).
### Add a Box
...
**✓ Checkpoint:** "Block Type: Cylinder" appears in the sidebar. The **Shape Chaining (Pipe Networks)** section also appears below.
...
With any mesh object selected (and not excluded via the **Exclude** toggle), the sidebar shows the per-object configuration. Here is what each section does.
### Cell Count
The **Cells X/Y/Z** field controls how many hexahedral cells are placed in each direction. For a box, these are X/Y/Z. For a cylinder, they are radial/tangential/axial.
### Patch Name
...
### Block Type
The **Block Type** dropdown lets you manually override how the block is interpreted. The options are **Box**, **Cylinder**, **Frustum**, **Extruded Ring**, **Wedge**, **Extrude**, **Revolve**, **Loft**. The primitives panel sets this automatically.
### Grading
...
- **SYMMETRIC**: Enter both a **Start Size** and **End Size**; cells are graded from both ends toward the middle.
### Boundary Conditions
The **Boundary Conditions** section lets you assign named OpenFOAM patches to specific faces.
1. Click the **+** (plus icon) button to add a patch entry.
2. Set the side name in the first field (e.g., `top`, `bottom`, `left`, `right` for a box).
3. Set the patch name in the second field (e.g., `inlet`).
4. Set the patch type in the third field (e.g., `wall`, `patch`, `symmetry`).
...
2. In the **STL Face Projection (Terrain Mapping)** section:
   - **Stl Projection Face**: choose which face to project (e.g., `top` or `bottom`).
   - **Stl File**: browse to the terrain STL.
### Step 3: Validate the STL
1. If an STL file is set, the **Validate STL** button will appear. Click it.
...
### Step 4: Generate
Click **1. Generate blockMeshDict** (or **▶ Run All**). The `blockMeshDict` will contain a `geometry {}` section and `project` entry on the selected face.
...
5. In Object Mode, set the **Extrude Axis** (X, Y, or Z) and **Extrude Distance** in the **Extrude Parameters** panel.
### Revolve from a Quad Face
Same as extrude — select a quad face, click **Tag as Revolve Block**. Then set the following in the **Revolve Parameters** panel:
...
**✓ Checkpoint:** "Tagged Faces 0 and 3 for Lofting" (or similar). The **Block Type** will be **Loft**.
---
...
### Convert the Sketch
1. With the `ClassySketch` object selected, find the **Sketch Actions** box in the sidebar. This box only appears for sketch curves.
...
4. In the **Shape Chaining (Pipe Networks)** section, use the **Chain From** object picker to select the first cylinder.
...
1. **1. Generate blockMeshDict** — writes `system/blockMeshDict`.
2. **2. Run blockMesh** — runs the OpenFOAM mesher.
3. **3. Convert to VTK** — converts the result with `foamToVTK`.
4. **4. Reload in Blender** — loads the VTK back into Blender as a new mesh object.
...
- **3. Convert to VTK**
- **4. Reload in Blender**
Each step checks that the previous step's output exists before running. If blockMesh hasn't run yet, "3. Convert to VTK" will say "polyMesh not found — run blockMesh first."
---
## Troubleshooting
